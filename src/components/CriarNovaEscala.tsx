...
import { Save, Plus, Trash2, ArrowLeft, AlertTriangle, Info, Loader2 } from 'lucide-react';
...
export function CriarNovaEscala({ userId, onSave, onBack }: CriarNovaEscalaProps) {
  ...
  const handleUpdateLinha = (id: string, field: keyof LinhaEscala, value: string) => {
    ...
    setLinhas(prev => prev.map(l => {
      if (l.id === id) {
        ...
        if (field === 'linha') {
          const dados = todasLinhasCadastradas.find(lc => lc.codigo === safeValue && lc.cliente_id === l.cliente);
    ...
    try {
      ...
      const { data: novaEscala, error: errorEscala } = await supabase
        ...
        .insert([{
          ...
          criado_por: userUUID,
          hora_criacao: new Date().toLocaleTimeString('pt-BR', { hour12: false })
        }])
        .select()
        .single();
      ...
      if (linhas.length > 0) {
        const viagensParaInserir = linhas.map(l => {
          const motoristaObj = motoristasCadastrados.find(m => m.nome === l.nomeMotorista);
          const clienteObj = clientesDisponiveis.find(c => c.id === l.cliente);
          const linhaObj = todasLinhasCadastradas.find(line => line.codigo === l.linha && line.cliente_id === l.cliente);
          ...
          return {
            ...
            fim: l.fim || null,
            // AJUSTE: Inclusão das colunas de deslocamento no salvamento
  ...
  const validarAntesDeSalvar = () => {
    ...
    const faltantes = todasLinhasCadastradas.filter(lc => !codigosNaEscala.includes(lc.codigo));
    ...
    <div className="min-h-screen bg-gray-100 p-4">
           ...
           <DialogHeader>
             ...
             <DialogDescription className="pt-4 text-slate-700">
               {linhasFaltantesParaExibir.length > 0 ? (
                <div>
                  ...
                  <div className="max-h-40 overflow-y-auto bg-slate-50 p-2 rounded border border-slate-200">
                    {linhasFaltantesParaExibir.map((lf, idx) => {
                      const cliente = clientesDisponiveis.find(c => c.id === lf.cliente_id);
                      return (
                        <div key={idx} className="text-xs py-1 border-b last:border-0 flex justify-between">
                          <span className="font-semibold">{cliente?.nome || 'Cliente'}</span>
                ...
                <p className="font-medium text-lg text-green-700 text-center">Deseja salvar a escala agora?</p>
              )}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex gap-2 justify-end mt-4">
            <Button variant="outline" onClick={() => setShowConfirmModal(false)}>Voltar</Button>
            <Button 
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={validarAntesDeSalvar} className="bg-green-600 hover:bg-green-700 text-white"><Save className="size-4 mr-2" /> Salvar Escala</Button>
        </div>
      </div>
      ...
      <Card className="mb-6 shadow-sm border-blue-100">
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 text-slate-700">
          ...
          <div>
            ...
            <Input value={diaSemana} readOnly className="bg-gray-50 font-bold text-blue-700" />
          </div>
          <div>
      ...
      <Card className="shadow-md overflow-hidden">
        <CardHeader className="flex flex-row items-center justify-between border-b bg-white p-4">
          ...
          <Button size="sm" onClick={handleAddLinha} className="bg-blue-600 hover:bg-blue-700 text-white"><Plus className="size-4 mr-1" /> Adicionar Linha</Button>
        </CardHeader>
          ...
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow>
                ...
                <TableHead className="min-w-[100px] text-orange-600">D. Inicial</TableHead>
                <TableHead className="min-w-[100px]">Início</TableHead>
                <TableHead className="min-w-[100px]">Fim</TableHead>
                <TableHead className="min-w-[100px] text-orange-600">D. Final</TableHead>
                ...
                <TableRow key={linha.id} className="hover:bg-slate-50">
                  <TableCell>
                    <Select value={linha.nomeMotorista || ''} onValueChange={val => handleUpdateLinha(linha.id, 'nomeMotorista', val)}>
                      <SelectTrigger className="h-8 text-xs bg-white border-slate-300"><SelectValue placeholder="Motorista" /></SelectTrigger>
                      <SelectContent className="bg-white">
                  ...
                  <TableCell>
                    <Select value={linha.cliente || ''} onValueChange={val => handleUpdateLinha(linha.id, 'cliente', val)}>
                      <SelectTrigger className="h-8 text-xs bg-white border-slate-300"><SelectValue placeholder="Cliente" /></SelectTrigger>
                      <SelectContent className="bg-white">
                  ...
                  <TableCell>
                    <Select value={linha.linha || ''} onValueChange={val => handleUpdateLinha(linha.id, 'linha', val)} disabled={!linha.cliente}>
                      <SelectTrigger className="h-8 text-xs bg-white border-slate-300"><SelectValue placeholder="Linha" /></SelectTrigger>
                      <SelectContent className="bg-white">
                        {todasLinhasCadastradas.filter(lc => lc.cliente_id === linha.cliente).map(lc => (
                          <SelectItem key={lc.id} value={lc.codigo}>{lc.codigo}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </TableCell>
                  <TableCell><Input value={linha.descricao || ''} readOnly className="bg-gray-50 h-8 text-xs border-slate-200" /></TableCell>
                  <TableCell>
                    <Select value={linha.sentido || ''} onValueChange={val => handleUpdateLinha(linha.id, 'sentido', val)}>
                      <SelectTrigger className="h-8 text-xs bg-white border-slate-300"><SelectValue placeholder="Sentido" /></SelectTrigger>
                      <SelectContent className="bg-white">
                        {sentidosBanco.map(s => <SelectItem key={s.id} value={s.codigo}>{s.descricao}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </TableCell>
                  <TableCell>
                    <Select value={linha.turno || ''} onValueChange={val => handleUpdateLinha(linha.id, 'turno', val)}>
                      ...
                      <SelectContent className="bg-white">
                        {turnosBanco.map((t) => (
                          <SelectItem key={t.id} value={t.codigo}>{t.descricao}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </TableCell>
                  {/* ADIÇÃO: Input Deslocamento Inicial */}
                  <TableCell>
                    <Input 
                      type="time" 
                  ...
                  {/* ADIÇÃO: Input Deslocamento Final */}
                  <TableCell>
                    <Input 
                      type="time" 
                  ...
                  <TableCell><Input value={linha.duracao || ''} readOnly className="bg-blue-50 font-bold h-8 text-xs text-blue-700 border-blue-200" /></TableCell>
                  <TableCell>
                    <Select value={linha.tipo || ''} onValueChange={val => handleUpdateLinha(linha.id, 'tipo', val)}>
                      <SelectTrigger className="h-8 text-xs bg-white border-slate-300"><SelectValue placeholder="Veículo" /></SelectTrigger>
                      <SelectContent className="bg-white">
                        {veiculosBanco.map(v => <SelectItem key={v.id} value={v.nome}>{v.nome}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </TableCell>
