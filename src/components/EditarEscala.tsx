...
// IMPORTAÇÕES ADICIONADAS PARA O DIALOG E ALERTAS
import { 
  Dialog, 
  DialogContent, 
  DialogHeader, 
...
import { Save, Plus, Trash2, ArrowLeft, Loader2, AlertTriangle, Info } from 'lucide-react';
...
export function EditarEscala({ escalaOriginal, onBack }: EditarEscalaProps) {
  ...
  const [carrosDB, setCarrosDB] = useState<any[]>([]); 
  ...
  // =========================
  // Não usamos mais o cálculo de pendências para exibir alerta.
  // Em vez disso, replicamos a validação de linhas ausentes por garagem.
  // =========================

  // =========================
  // FUNÇÕES AUXILIARES (SUA LÓGICA DE NEGÓCIO ORIGINAL)
  // =========================
  ...
  const handleUpdateLinha = (id: string, field: keyof LinhaEscala, value: string) => {
    setLinhas(prev => prev.map(l => {
      if (l.id === id) {
        ...
        if (field === 'cliente') { up.linha = ''; up.descricao = ''; }
        if (field === 'linha') {
  ...
  // Validação antes de salvar: verifica campos obrigatórios e lista linhas da garagem que não foram adicionadas à escala.
  const validarAntesDeSalvar = () => {
    ...
    const linhasDaGaragem = todasLinhasDB.filter(
  ...
  // Executa o salvamento final reutilizando a função de edição. Encerra o modal após concluir.
  ...
  const salvarEdicao = async () => {
    ...
    setIsSaving(true);
    try {
      const { error: errE } = await supabase.from('escalas').update({ data_escala: dataEscala, garagem_id: garagemId, dia_semana_texto: diaSemana }).eq('id', escalaOriginal.id);
      if (errE) throw errE;
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      {/* Não exibimos banner de pendências; a validação de linhas ausentes ocorrerá ao salvar. */}
                ...
                <TableRow key={linha.id} className="hover:bg-slate-50">
                  <TableCell>
                    <Select value={linha.nomeMotorista} onValueChange={val => handleUpdateLinha(linha.id, 'nomeMotorista', val)}>
                      ...
                      <SelectContent className="bg-white">
                        {motoristasDB.filter(m => !garagemId || m.garagem_id === garagemId).map(m => <SelectItem key={m.id} value={m.nome}>{m.nome}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Select value={linha.linha} onValueChange={val => handleUpdateLinha(linha.id, 'linha', val)} disabled={!linha.cliente}>
                      <SelectTrigger className="h-8 text-xs bg-white"><SelectValue /></SelectTrigger>
                      <SelectContent className="bg-white">
                        {todasLinhasDB.filter(lc => lc.cliente_id === linha.cliente && lc.garagem_id === garagemId).map(lc => (
                  ...
                  <TableCell>
                    <Select value={linha.tipo || ''} onValueChange={(val) => {
                      ...
                      <SelectContent className="bg-white">
                        ...
                        {linha.nomeMotorista ? (() => {
                                ...
                                return permitidos.length === 0 ? <SelectItem value="__none" disabled>Nenhum permitido</SelectItem> : permitidos.map(v => <SelectItem key={v.id} value={v.nome}>{v.nome}</SelectItem>);
                            })() : <SelectItem value="__dis" disabled>Selecione motorista</SelectItem>}
                      </SelectContent>
                    </Select>
                  </TableCell>
                  <TableCell>
                    <Select value={linha.carro || SEM_CARRO_VALUE} onValueChange={(val) => {
                      ...
                      <SelectContent className="bg-white">
                        ...
                        {linha.nomeMotorista ? (() => {
                                ...
                                if (!tipoSelecionadoId || tiposPermitidos.length === 0) return <SelectItem value="__none" disabled>Indisponível</SelectItem>;
                                const veiculosDisponiveis = carrosDB.filter(c => c.garagem_id === garagemId && c.tipo_veiculo_id === tipoSelecionadoId && (c.ativo === null || c.ativo === true));
                                return veiculosDisponiveis.length === 0 ? <SelectItem value="__none" disabled>Sem carros</SelectItem> : veiculosDisponiveis.map(c => <SelectItem key={c.id} value={c.prefixo}>{c.prefixo}</SelectItem>);
                            })() : <SelectItem value="__dis" disabled>Selecione motorista</SelectItem>}
                      </SelectContent>
                    </Select>
                  </TableCell>
